              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When this is the final answer, show special hint -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll to keep button at same viewport position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract core entities - full extraction, with summary and related fact count
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current valid facts - full extraction, no limit
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **Alumni_345(index=1)**: As a university alumni...
        ...
        // Example: - select Parent_601(index 0): As a representative of parents...
        if (!headerMatch) {
        ...
        // Example: - **Parent_601(index 0)**: As a representative of parents...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation of reason (exclude ending summary paragraph)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first question has "1." prefix, needs special handling
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform marker (very old format), use entire text as answer
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" in Chinese format (backend new format)
      // 2. "1. " or "\n1. " (number+dot, old format compatible)
      let matches = []
      let match

      // Try Chinese "Question X:" format first
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 heading (## xxx) since section title is shown in outer layer
  ...
  // Process line breaks - blank lines become paragraph breaks, single breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Fix non-consecutive ordered list numbering: keep incrementing when single-item <ol> blocks are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Chinese "Final Answer:"
  ...
  // If content is long and contains markdown, try removing thinking process before returning
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dark gray, does not change with state */
  font-weight: 500;
}
