...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to cluster isolated node groups toward the center
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc from the node and back
      const loopRadius = 30
      // Start from the right side of the node, loop back
    ...
    // Calculate curve control point - dynamically adjusted based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset to the edge direction, calculated proportionally to distance
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // First time actual drag detected, restart simulation
          d._isDragging = true
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (no rotation, horizontal display for clarity)
...
// Watch edge labels visibility toggle
