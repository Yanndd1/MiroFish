        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
            ...
            <div v-if="showToolsDetail" class="tools-card-body">
              <div class="tools-grid">
                ...
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $t('step5.interviewSubAgent') }}</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? $t('step5.chatEmptyReport') : $t('step5.chatEmptyAgent') }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              ...
              <span class="results-count">{{ $t('step5.replyCount', { count: surveyResults.length }) }}</span>
            </div>
...
const selectAgent = (agent, idx) => {
  ...
  addLog(t('step5.logSelectTarget', { name: agent.username }))
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(t('step5.logSendFailed', { error: err.message }))
    chatHistory.value.push({
      role: 'assistant',
...
const sendToReportAgent = async (message) => {
  addLog(t('step5.logSendToReportAgent', { message: message.substring(0, 50) }))
  ...
  if (res.success && res.data) {
    ...
    addLog(t('step5.logReportAgentReplied'))
  } else {
...
const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error(t('step5.pleaseSelectAgent'))
  }

  addLog(t('step5.logSendToAgent', { name: selectedAgent.value.username, message: message.substring(0, 50) }))
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .join('\n')
    prompt = t('step5.chatContextPrompt', { history: historyContext, message: message })
  }
  ...
  if (res.success && res.data) {
    ...
    if (responseContent) {
      ...
      addLog(t('step5.logAgentReplied', { name: selectedAgent.value.username }))
    } else {
...
const submitSurvey = async () => {
  ...
  addLog(t('step5.logSendingSurvey', { count: selectedAgents.value.size }))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step5.logReceivedReplies', { count: surveyResults.value.length }))
    } else {
      throw new Error(res.error || t('step5.requestFailed'))
    }
  } catch (err) {
    addLog(t('step5.logSurveyFailed', { error: err.message }))
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    addLog(t('step5.logLoadingReport', { id: props.reportId }))
    ...
    addLog(t('step5.logLoadReportFailed', { error: err.message }))
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step5.logReportDataLoaded'))
    }
  } catch (err) {
    addLog(t('step5.logLoadLogsFailed', { error: err.message }))
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step5.logProfilesLoaded', { count: profiles.value.length }))
    }
  } catch (err) {
    addLog(t('step5.logLoadProfilesFailed', { error: err.message }))
  }
}
