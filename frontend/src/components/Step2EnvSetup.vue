      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        <div class="card-header">
          ...
          <div class="step-status">
            ...
            <span v-else class="badge processing">{{ $t('step2.init') }}</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              ...
              <span class="info-value mono">{{ taskId || $t('step2.taskComplete') }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.genAgentProfile') }}</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">{{ $t('step2.seedTopicCount') }}</span>
            </div>
          </div>
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <span class="preview-title">{{ $t('step2.generatedProfiles') }}</span>
            </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.genDualPlatformConfig') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 2" class="badge success">{{ $t('common.completed') }}</span>
            <span v-else-if="phase === 2" class="badge processing">{{ $t('common.processing') }}</span>
            <span v-else class="badge pending">{{ $t('common.pending') }}</span>
          </div>
        </div>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-block-header">
                ...
                <span class="config-block-badge">{{ simulationConfig.agent_configs?.length || 0 }} {{ $t('common.unit') }}</span>
              </div>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">{{ $t('step2.algoConfig') }}</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.step04Title') }}</span>
          </div>
          <div class="step-status">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.step05Title') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase >= 4" class="badge processing">{{ $t('common.processing') }}</span>
            <span v-else class="badge pending">{{ $t('common.pending') }}</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <p class="description">{{ $t('step2.simEnvReady') }}</p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc" v-html="$t('step2.roundsConfigDesc', { hours: simulationConfig?.time_config?.total_simulation_hours || '-', minutes: simulationConfig?.time_config?.minutes_per_round || '-' })"></span>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">{{ $t('step2.displayAge') }}</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} {{ $t('step2.yearsOld') }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ $t('step2.displayGender') }}</span>
              <span class="info-value">{{ { male: $t('step2.male'), female: $t('step2.female'), other: $t('step2.otherGender') }[selectedProfile.gender] || selectedProfile.gender }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ $t('step2.countryRegion') }}</span>
              <span class="info-value">{{ selectedProfile.country || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ $t('step2.displayMBTI') }}</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            <span class="section-label">{{ $t('step2.detailedBackground') }}</span>
            
            <!-- 人设维度概览 -->
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.eventExperience') }}</span>
                <span class="dim-desc">{{ $t('step2.eventExpDesc') }}</span>
              </div>
              <div class="dimension-card">
              ...
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.uniqueMemory') }}</span>
                <span class="dim-desc">{{ $t('step2.uniqueMemoryDesc') }}</span>
              </div>
              <div class="dimension-card">
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'generating_profiles') {
    ...
    // 进入配置生成阶段，开始轮询配置
    if (!configTimer) {
      addLog(t('step2.genDualPlatformConfigStart'))
      startConfigPolling()
    }
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(t('step2.startSimCustom', { rounds: customMaxRounds.value }))
  } else {
    // 用户选择保持自动生成的轮数，不传递 max_rounds 参数
    addLog(t('step2.startSimAuto', { rounds: autoGeneratedRounds.value }))
  }
...
const startPrepareSimulation = async () => {
  if (!props.simulationId) {
    addLog(t('step2.errorMissingSimId'))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog(t('step2.existingPrepFound'))
        await loadPreparedData()
        return
      }
      
      taskId.value = res.data.task_id
      addLog(t('step2.prepareTaskStarted'))
      addLog(`  └─ Task ID: ${res.data.task_id}`)
      
      // 立即设置预期Agent总数（从prepare接口返回值获取）
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(t('step2.entitiesFromGraph', { count: res.data.expected_entities_count }))
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ ${t('step2.entityTypes')}: ${res.data.entity_types.join(', ')}`)
        }
      }
      
      addLog(t('step2.startPolling'))
      // 开始轮询进度
      startPolling()
      // 开始实时获取 Profiles
      startProfilesPolling()
    } else {
      addLog(t('step2.prepareFailed', { error: res.error || t('common.unknownError') }))
      emit('update-status', 'error')
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        addLog(t('step2.prepareComplete'))
        stopPolling()
        stopProfilesPolling()
        await loadPreparedData()
      } else if (data.status === 'failed') {
        addLog(t('step2.prepareFailedStatus', { error: data.error || t('common.unknownError') }))
        stopPolling()
        stopProfilesPolling()
      }
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(t('step2.startGenProfiles'))
        }
        addLog(t('step2.agentProfileProgress', { current: currentCount, total: total, name: profileName, profession: latestProfile?.profession || t('step2.unknownProfession') }))
        
        // 如果全部生成完成
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(t('step2.allProfilesComplete', { count: currentCount }))
        }
      }
    }
  } catch (err) {
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog(t('step2.genProfileConfig'))
        } else if (data.generation_stage === 'generating_config') {
          addLog(t('step2.genSimConfigParams'))
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        addLog(t('step2.simConfigComplete'))
        
        // 显示详细配置摘要
        if (data.summary) {
          addLog(`  ├─ ${t('step2.summaryAgents', { count: data.summary.total_agents })}`)
          addLog(`  ├─ ${t('step2.summaryDuration', { hours: data.summary.simulation_hours })}`)
          addLog(`  ├─ ${t('step2.summaryPosts', { count: data.summary.initial_posts_count })}`)
          addLog(`  ├─ ${t('step2.summaryTopics', { count: data.summary.hot_topics_count })}`)
          addLog(`  └─ ${t('step2.summaryPlatforms', { twitter: data.summary.has_twitter_config ? '✓' : '✗', reddit: data.summary.has_reddit_config ? '✓' : '✗' })}`)
        }
        
        // 显示时间配置详情
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(t('step2.timeConfigLog', { minutesPerRound: tc.minutes_per_round, totalRounds: Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round) }))
        }
        
        // 显示事件配置
        if (data.config.event_config?.narrative_direction) {
          const narrative = data.config.event_config.narrative_direction
          addLog(t('step2.narrativeLog', { text: narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative }))
        }
        
        stopConfigPolling()
        phase.value = 4
        addLog(t('step2.envSetupComplete'))
...
const loadPreparedData = async () => {
  phase.value = 2
  addLog(t('step2.loadingExistingConfig'))

  // 最后获取一次 Profiles
  await fetchProfilesRealtime()
  addLog(t('step2.loadedProfiles', { count: profiles.value.length }))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog(t('step2.configLoadSuccess'))
        
        // 显示详细配置摘要
        if (res.data.summary) {
          addLog(`  ├─ ${t('step2.summaryAgents', { count: res.data.summary.total_agents })}`)
          addLog(`  ├─ ${t('step2.summaryDuration', { hours: res.data.summary.simulation_hours })}`)
          addLog(`  └─ ${t('step2.summaryPosts', { count: res.data.summary.initial_posts_count })}`)
        }
        ...
        // 配置尚未生成，开始轮询
        addLog(t('step2.configGenerating'))
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(t('step2.configLoadFailed', { error: err.message }))
    emit('update-status', 'error')
  }
}
