    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('step3.startingBtn') : $t('step3.generateReport') }}
...
const doStartSimulation = async () => {
  ...
  startError.value = null
  addLog(t('step3.startingDualSim'))
  ...
  try {
    ...
    addLog(t('step3.graphUpdateEnabled'))
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog(t('step3.oldSimCleaned'))
      }
      ...
      addLog(t('step3.startFailed', { error: res.error || t('step1.unknownError') }))
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(t('step3.startException', { error: err.message }))
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(t('step3.stopFailed', { error: res.error || t('step1.unknownError') }))
    }
  } catch (err) {
    addLog(t('step3.stopException', { error: err.message }))
  } finally {
    isStopping.value = false
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog(t('step3.allPlatformsCompleted'))
        }
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog(t('step3.reportAlreadySent'))
    return
  }

  isGeneratingReport.value = true
  addLog(t('step3.startingReport'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step3.reportTaskStarted', { reportId }))
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(t('step3.reportStartFailed', { error: res.error || 'unknown' }))
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(t('step3.reportStartException', { error: err.message }))
    isGeneratingReport.value = false
  }
}
