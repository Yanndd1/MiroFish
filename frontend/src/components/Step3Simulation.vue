...
const doStartSimulation = async () => {
  ...
  // Reset all state first to ensure no carryover from previous simulation
  resetAllState()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Additional check: platforms may report completion before backend updates runner_status
      // Detected via twitter_completed and reddit_completed status
...
const checkPlatformsCompleted = (data) => {
  ...
  // Check each platform completion status
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // No auto-scroll, let user browse timeline freely
