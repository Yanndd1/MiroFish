...
const handleNextStep = (params = {}) => {
  addLog(t('logs.enterStep', { step: 3, name: t('steps.startSim') }))
  
  // 记录模拟轮数配置
  if (params.maxRounds) {
    addLog(t('logs.customRounds', { rounds: params.maxRounds }))
  } else {
    addLog(t('simView.useAutoRounds'))
  }
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          addLog(t('simView.closeEnvFailed', { error: closeRes.error || t('step1.unknownError') }))
          // 如果优雅关闭失败，尝试强制停止
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(t('simView.closeEnvException', { error: closeErr.message }))
        // 如果优雅关闭异常，尝试强制停止
        await forceStopSimulation()
      }
    } else {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog(t('simView.simRunningStop'))
        await forceStopSimulation()
      }
    }
  } catch (err) {
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      ...
      addLog(t('simView.forceStopFailed', { error: stopRes.error || t('step1.unknownError') }))
    }
  } catch (err) {
    addLog(t('simView.forceStopException', { error: err.message }))
  }
}
    ...
    if (simRes.success && simRes.data) {
      ...
      if (simData.project_id) {
        ...
        if (projRes.success && projRes.data) {
          ...
          addLog(t('simView.projectLoadSuccess', { id: projRes.data.project_id }))
      ...
      addLog(t('simView.loadSimDataFailed', { error: simRes.error || t('step1.unknownError') }))
    }
  } catch (err) {
...
const loadGraph = async (graphId) => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(t('simView.graphDataLoadSuccess'))
    }
  } catch (err) {
