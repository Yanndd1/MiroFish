...
if sys.platform == 'win32':
    ...
    # Force set default encoding (affects the default encoding of open() function)
...
# Load .env file from project root (contains LLM_API_KEY and other configs)
...
# Add filter immediately at module load time, ensuring it takes effect before camel code executes
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview on both platforms, return consolidated results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden by each interview item)
                - "twitter": Interview on Twitter platform only
                - "reddit": Interview on Reddit platform only
                - None/unspecified: Interview each Agent on both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of placeholders like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid value from last read (using rowid instead of created_at because different platforms have different created_at formats)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: add original post content and author
            ...
            if new_post_id:
                # The reposted post's original_post_id points to the original post
                cursor.execute("""
        ...
        # Context enrichment failure doesn't affect the main flow
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prefer using names from agent_names
            author_name = ''
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configuration for speeding up parallel simulation:
...
async def run_twitter_simulation(
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed row number in database (using rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end even when no active agents (actions_count=0)
            if action_logger:
    ...
    # Note: don't close the environment, keep it for Interview use
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, otherwise falls back to general configuration)
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure proper exit when SIGTERM/SIGINT is received

    Persistent simulation scenario: don't exit after simulation, wait for interview commands
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (give loop a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
        
        # Don't call sys.exit() directly, let asyncio loop exit normally and clean up resources
