...
def create_simulation():
    ...
    Note: Parameters like max_rounds are intelligently generated by the LLM; no manual setup required.
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in backend/scripts/ directory and are no longer copied to the simulation directory.
    ...
    # Required file list (excluding scripts, which are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - running: Currently running, so preparation was completed earlier
        # - completed: Run finished, so preparation was completed earlier
        # - stopped: Stopped, so preparation was completed earlier
...
def prepare_simulation():
    ...
    Features:
    - Automatically detects completed preparation work to avoid duplication
    ...
    try:
        ...
        # Check if preparation is already complete (avoid duplicate generation)
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Don't get edge info, for faster speed
            )
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task doesn't exist, but if simulation_id is provided, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list with rich info
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds; if not available, use recommended rounds
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (may be in the middle of writing): {e}")
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Real-time retrieval of simulation configuration (for viewing progress during generation)

    Differences from /config endpoint:
    - Reads files directly, bypassing SimulationManager
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g., file modification time, whether generation is in progress)
    - Can return partial info even if configuration hasn't been fully generated
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (may be in the middle of writing): {e}")
                config = None
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "force": false                         // Optional: Force restart (stops running simulation and cleans up logs)
        }
    ...
    About the force parameter:
        ...
        - Does NOT clean configuration files (simulation_config.json) and profile files
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation complete, check if there's a running process
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows content from both platforms for the current latest round
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend display of Agent activity rankings, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    (entered waiting-for-command mode after completing simulation loop)
...
def close_simulation_env():
    ...
    Sends a close environment command to the simulation for graceful shutdown
    and exit from waiting-for-command mode.
