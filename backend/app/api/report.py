...
def check_report_status(simulation_id: str):
    """
    Check if simulation has a report, and the report status
...
def get_agent_log(report_id: str):
    ...
    Real-time retrieval of each action step during report generation, including:
    - Report start, planning start/complete
    - Each section's start, tool calls, LLM responses, completion
    - Report completion or failure

    Query parameters:
        from_line: Start reading from line number (optional, default 0, for incremental retrieval)
...
def get_console_log(report_id: str):
    ...
    this is plain-text console-style log output.
