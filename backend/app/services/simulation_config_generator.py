...
Uses a step-by-step generation strategy to avoid failures from generating too much content at once:
...
# China timezone schedule configuration (Beijing time)
...
class EventConfig:
    ...
    # Narrative direction for public opinion
...
### Basic Principles (for reference only, should be flexibly adjusted based on specific events and participating groups):
- Please infer the timezone and daily schedule habits of the target user group based on the simulation scenario. The following is a reference example for UTC+8 (East Asia)
...
- **Important**: The following example values are for reference only. You need to adjust specific time periods based on the nature of the event and characteristics of participating groups
  - For example: student groups may peak at 9-11 PM; media active all day; official institutions only during work hours
  - For example: breaking news may lead to late-night discussions, off_peak_hours can be shortened accordingly
...
- peak_hours (int array): Peak hours, adjusted based on event participating groups
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration should match the daily schedule habits of the target user group in the simulation scenario."
...
- **Time should match target user group's daily schedule**: The following is a reference (UTC+8), please adjust based on simulation scenario
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, considering target group's daily schedule>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration should match the daily schedule habits of the target user group in the simulation scenario."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Experts/Professors: active during work hours + evenings, moderate frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
