...
Converts entities from Zep graphs into Agent Profile format required by the OASIS simulation platform
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep graphs into Agent Profiles required for OASIS simulation
    ...
    # Group/institution entity types (require generating representative group personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search API, so we need to search edges and nodes
        separately and then merge the results. Parallel requests are used for efficiency.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        if zep_results.get("node_summaries"):
            context_parts.append("### Related Nodes Retrieved via Zep\n" + "\n".join(f"- {s}" for s in zep_results["node_summaries"][:10]))
        
        return "\n\n".join(context_parts)
    
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determine if this is an individual entity type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if this is a group/institution entity type"""
    ...
    def _generate_profile_with_llm(
        ...
        - Group/institution entity: Generate representative account profile
        """
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user profiles. Generate detailed, realistic personas for public opinion simulation, restoring existing real-world situations as closely as possible. You must return valid JSON format, and all string values must not contain unescaped newline characters."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, restoring existing real-world situations as closely as possible.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Stances and opinions (attitude toward topics, content that might provoke anger/emotion)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institution/group entity, restoring existing real-world situations as closely as possible.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   ...
   - Institutional memory (important part of the persona, describe this institution's connection to the event, and its existing actions and reactions in the event)
...
- Institutional account communication must align with its identity positioning"""
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Must include user_id field, which is critical for OASIS agent_graph.get_agent() matching!
