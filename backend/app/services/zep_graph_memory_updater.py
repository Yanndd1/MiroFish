...
Dynamically update Agent activities from simulations to the Zep graph
"""
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action log files and updates new agent activities to Zep graph in real-time.
    Groups by platform, sends in batches to Zep after accumulating BATCH_SIZE activities.

    All meaningful actions are updated to Zep, action_args contain full context information:
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Activity buffers grouped by platform (each platform accumulates up to BATCH_SIZE before batch sending)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contain full context information (e.g., original post text, user names, etc.).
    ...
    def _worker_loop(self, locale: str = 'zh'):
        """Background worker loop - batch send activities to Zep grouped by platform"""
        set_locale(locale)
        while self._running or not self._activity_queue.empty():
            try:
                ...
                try:
                    ...
                    # Add activity to the corresponding platform's buffer
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "skipped_count": self._skipped_count,        # Number of filtered/skipped activities (DO_NOTHING)
...
class ZepGraphMemoryManager:
    """
    Manager for multiple simulation Zep graph memory updaters
