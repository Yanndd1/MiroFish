...
def _to_pascal_case(name: str) -> str:
    ...
    # Further split by camelCase boundaries (e.g., 'camelCase' -> ['camel', 'Case'])
    words = []
    for part in parts:
...
- We need to simulate the reactions of various parties in public opinion events and information propagation paths
...
A. **Fallback types (must be included, placed as last 2 in the list)**:
   - `Person`: Fallback type for any natural person. When a person does not belong to any more specific person type, they are classified here.
   - `Organization`: Fallback type for any organization. When an organization does not belong to any more specific organization type, it is classified here.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types for the main roles appearing in the text
   - For example: if the text involves academic events, there can be `Student`, `Professor`, `University`
   - For example: if the text involves business events, there can be `Company`, `CEO`, `Employee`
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Record mapping from original names to PascalCase, used for fixing edge source_targets references later
        entity_name_map = {}
        for entity in result["entity_types"]:
            # Force convert entity name to PascalCase (required by Zep API)
            if "name" in entity:
        ...
        for edge in result["edge_types"]:
            # Force convert edge name to SCREAMING_SNAKE_CASE (required by Zep API)
            if "name" in edge:
        ...
        # Deduplicate: deduplicate by name, keep first occurrence
        seen_names = set()
        deduped = []
        for entity in result["entity_types"]:
