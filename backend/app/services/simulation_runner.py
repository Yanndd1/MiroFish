...
Run simulations in the background and record each Agent's actions, with real-time status monitoring
"""
...
class SimulationRunner:
    ...
    3. Providing real-time state query interface
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, for truncating overly long simulations)
        ...
        try:
            # Build run command with full path
            # New log structure:
            ...
            # Create main log file to avoid process blocking due to stdout/stderr pipe buffer overflow
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables, ensure UTF-8 encoding on Windows
            # This fixes encoding issues when third-party libraries (e.g., OASIS) read files without specifying encoding
            env = os.environ.copy()
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensuring all related processes can be terminated when server shuts down
            )
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform was running, only check that platform
                                    # If two platforms were running, both need to complete
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for forcing a restart)
    ...
    def cleanup_all_simulations(cls):
        ...
        Called when server shuts down, ensuring all child processes are terminated
        """
        ...
        # Check if there is content to clean up (avoid printing useless logs for empty processes)
    ...
    def register_cleanup(cls):
        ...
        Called at Flask app startup, ensuring all simulation processes are cleaned up when server shuts down
        """
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (the process actually running the app)
        # WERKZEUG_RUN_MAIN=true indicates the reloader subprocess
        # In non-debug mode, this env var does not exist, also need to register
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered, prevent subprocess from retrying
            return
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform simulation, interview both platforms and return integrated results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: In dual-platform simulation, interview both platforms for each Agent
            timeout: Timeout in seconds
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation, making it gracefully exit command-wait mode
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close environment command sent (response timeout, environment may be shutting down)"
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specify Agent ID (optional, get history for this Agent only)
