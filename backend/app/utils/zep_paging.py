"""Zep Graph paginated reading utility.

Zep's node/edge list API uses UUID cursor pagination.
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request with exponential backoff retry on failure. Only retries transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, returning at most max_items entries (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination, returning the complete list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
